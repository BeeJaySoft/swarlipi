...
Per script:
  ...
  5. Rename (Noto is an OFL Reserved Font Name), lift the ascender so a mark
     line fits the line box, write the TTF (the install format; nothing embeds
...
def komal_variants(font: TTFont, komal: str, script: str) -> list[str]:
...
def build_one(cache: Path, script: str, out: Path) -> Path:
    ...
    font.save(ttf)
    print(
        f"  {ttf.name}: above line {above_line}, {len(base_glyphs(font))} bases anchored, {ttf.stat().st_size // 1024} KB"
    )
    return ttf


def write_license(cache: Path, out: Path) -> None:
    with zipfile.ZipFile(fetch(cache, "gurmukhi")) as zf:
